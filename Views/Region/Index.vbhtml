...
<div class="flex justify-between items-center mb-6">
    <h2 class="text-3xl font-bold text-white">Regiones</h2>
    <button onclick="abrirModalNuevo()" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg flex items-center gap-2 transition-colors">
        <i data-lucide="plus-circle" class="w-5 h-5"></i> Crear Región
    </button>
</div>

<div class="card p-4 rounded-lg overflow-x-auto">
    <table id="tablaRegiones" class="w-full text-left">
        <thead class="table-header">
            <tr>
                <th class="p-4 font-semibold">Nombre</th>
                <th class="p-4 font-semibold">Tipo de Región</th>
                <th class="p-4 font-semibold">País</th>
...
<div id="modalRegion" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 p-4">
    <div id="modalRegionContent" class="bg-gray-800 w-full max-w-md rounded-lg shadow-lg p-8 transform transition-all scale-95 opacity-0">
        <h2 class="text-2xl font-bold mb-6 text-white" id="tituloModal">Crear Región</h2>
        <form id="formRegion">
            <input type="hidden" id="txtRegionID" />
            ...
            <div class="mb-6">
                <label for="txtTipoRegion" class="block text-sm font-medium text-gray-300 mb-2">Tipo de Región</label>
                <input type="text" id="txtTipoRegion" class="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500" required />
            </div>
...
@section Scripts
    ...
    <script src="~/Scripts/js/region.js"></script>
End Section